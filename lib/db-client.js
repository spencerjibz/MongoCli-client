...
dbClient.connect(url,{useUnifiedTopology:true},(err,client)=>{
...
exports.findAll = function(db,col){
  ...
  dbClient.connect(`${url}/${db}`,{useUnifiedTopology:true},function(err,client){
     if( err) {log(`Connection failed or not mongo service is running if its not the case, check your dbname and collection name`.yellow.bold)}
...
db.listCollections().toArray(function(err,docs){
    if(err) throw err
     ...
     /// close the client created from conection
     client.close()
...
exports.findOnly = function(db,col,filter){
 mongoose.connect(`${url}/${db}`,{useNewUrlParser:true,useUnifiedTopology:true},(err,db)=>{
if(err) {
...
exports.cols = function(dbname){
    // connected to the 
    dbClient.connect(`${url}/${dbname}`,{useUnifiedTopology:true}, (err, db) => {
     // err handling
     if(err ) {
...
exports.UpdateOne = (dbname,col,filter,update)=>{
    dbClient.connect(`${url}/${dbname}`, (err, client) => {

        err ? log('connection failed'.bgRed) :

            client.eval(`db.getCollection('${col}').findOneAndUpdate(${filter},${update})`, (err, done) => {
                if (err) {
                    log('failed to update the document specified',err)
                    process.exit()
                }
                else {
                    log('document updated successfully'.bgBlue)
