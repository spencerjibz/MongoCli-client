...
Run('mongo',[`${lswin?url:process.cwd()}`],Shellpath,(err,db,stderr)=>{
    if(err) {
...
exports.delCol = (dbname,colname)=>{
    dbClient.connect(`${url}`,(err,client)=>{
        ...
        client.db(dbname).collection(colname).drop((err,done)=>{
            ...
            log(`${colname} was successfully deleted`.blue)
            exports.cols(dbname)
            client.close()
                }        })
    })
...
dbClient.connect(`${url}/${dbname}`,(err,client)=>{
    ...
    client.eval(`db.getCollection('${col}').insertMany(${obj})`,(err,done)=>{
       if(err) {
           log('failed to insert the document specified',err)
        }
...
exports.createDb = (dbname)=> {
    ...
    dbClient.connect(`${url}/${dbname}`,(err,db)=>{
        ...
        :
        db.eval(`db.getSiblingDB('${dbname})')`,(err,done)=>{
            err? log(`failed to create ur named database`.red):
            
            log('database successfully created')
          let  coll = promisify(exports.createCol)
         
       coll(dbname,'test').finally((e)=>{
